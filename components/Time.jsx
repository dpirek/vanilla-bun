function AnalogClock({date}) {
  const hrPosition = ((date.getHours() * 360) / 12 + (date.getMinutes() * (360 / 60)) / 12) + 3 / 360;
  const minPosition = ((date.getMinutes() * 360) / 60 + (date.getSeconds() * (360 / 60)) / 60) + 6 / 60;
  const secPosition = ((date.getSeconds() * 360) / 60) + 6;
  ...
  return <svg id="clock" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 600">
    ...
    <g id="minute" style={{transform: 'rotate(' + minPosition + 'deg)'}}>
      <path className="minute-arm" d="M300.5 298V67" />
      <circle className="sizing-box" cx="300" cy="300" r="253.9" />
    </g>
    <g id="second" style={{transform: 'rotate(' + secPosition + 'deg)'}}>
